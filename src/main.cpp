...
public:
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt)
	{
		...
		transform.rotation = atan2f(vel.y, vel.x) * RAD2DEG + 90.0f;
	}
	bool Update(float dt) {
		transform.position = Vector2Add(transform.position, Vector2Scale(physics.velocity, dt));
	...
	void Draw() const {
		switch (type) {
			...
			case WeaponType::TRIPLE : {
				static constexpr float TRIPLE_LENGTH = 15.f;
				static constexpr float TRIPLE_WIDTH = 4.f;

				// Punkty względem środka (skierowane w górę)
				Vector2 tip = { 0, -TRIPLE_LENGTH };
				Vector2 left = { -TRIPLE_WIDTH / 2, 0 };
				Vector2 right = { TRIPLE_WIDTH / 2, 0 };

				// Obracanie punktów
				float radians = transform.rotation * DEG2RAD;
				Vector2 rotatedTip = Vector2Rotate(tip, radians);
				Vector2 rotatedLeft = Vector2Rotate(left, radians);
				Vector2 rotatedRight = Vector2Rotate(right, radians);

				// Przesunięcie do pozycji świata
				Vector2 pos = transform.position;
				DrawTriangle(
					Vector2Add(pos, rotatedTip),
					Vector2Add(pos, rotatedLeft),
					Vector2Add(pos, rotatedRight),
					ORANGE
				);
				break;
			}
	...
	float speed)
{
	Vector2 vel{ 0, -speed };
	switch (wt) {
		case WeaponType::LASER: return Projectile(pos, vel, 20, wt);
		case WeaponType::BULLET: return Projectile(pos, vel, 10, wt);
		case WeaponType::TRIPLE: default: return Projectile(pos, vel, 5, wt);
	}
}
...
public:
	Ship(int screenW, int screenH) {
		...
		fireRateBullet = 10.f;
		fireRateTriple = 3.f;
		spacingLaser = 50.f; // px between lasers
		spacingBullet = 30.f;
		spacingTriple = 100.f;
	}
	...
	float GetSpacing(WeaponType wt) const {
		switch (wt) {
			...
			case WeaponType::TRIPLE: return spacingTriple;
		}
	}
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
				...
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						if (currentWeapon == WeaponType::TRIPLE) {
							float angles[] = { -20.f, 0.f, 20.f };
							for (float angle : angles) {
								Vector2 direction = Vector2Rotate({ 0,-1 }, angle * DEG2RAD);
								Vector2 velocity = Vector2Scale(direction, projSpeed);
								projectiles.emplace_back(p, velocity, 5, WeaponType::TRIPLE);
							}
						}
						else {
			...
			// Render everything
			{
				...
				const char* weaponName = (currentWeapon == WeaponType::LASER) ? "LASER" : (currentWeapon == WeaponType::BULLET) ? "BULLET" : "TRIPLE";
